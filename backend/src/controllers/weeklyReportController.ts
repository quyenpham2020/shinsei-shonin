...
export const createOrUpdateReport = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (existingReport) {
      ...
      const updatedReport = await getOne('SELECT * FROM weekly_reports WHERE id = $1', [existingReport.id]);
      ...
      const newReport = await getOne('SELECT * FROM weekly_reports WHERE id = $1', [result.rows[0].id]);
...
export const getMembersReportsLast3Weeks = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    let membersQuery = `SELECT id, employee_id, name, email, department FROM users WHERE (weekly_report_exempt IS NULL OR weekly_report_exempt = false)`;
    ...
    const reportPlaceholders = weekStarts.map((_, i) => `$${i + 1}`).join(',');

    const reportsQuery = `
      SELECT id, user_id, week_start_date, week_end_date, content, achievements, challenges, next_week_plan, updated_at
      FROM weekly_reports
      WHERE week_start_date IN (${reportPlaceholders})
    ...
    reports.forEach(r => {
      reportMap.set(`${r.user_id}-${r.week_start_date}`, r);
    });
